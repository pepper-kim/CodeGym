...
/**
 * 컴퓨터의 전략적 위치 선택 함수 (공격 → 수비 → 중앙 → 코너 → 랜덤 순)
 *
 * - 1단계: 컴퓨터가 이길 수 있는 위치가 있다면 즉시 선택
 * - 2단계: 상대가 이길 수 있는 위치가 있다면 즉시 차단
 * - 3단계: 중앙(5번 자리)이 비어 있다면 선점
 * - 4단계: 코너 자리(1, 3, 7, 9) 중 비어 있는 곳을 우선 선택
 * - 5단계: 위 모든 조건이 없으면 랜덤한 빈 칸을 선택
 *
 * 이 AI는 기본적인 공격과 방어, 중앙 선점과 코너 우선 전략을 모두 고려하여
 * 보다 강력하고 인간적인 플레이를 흉내냅니다.
 *
 * @param {string[]} board - 현재 보드 배열 (index 1~9 사용)
 * @param {string} computerMarker - 컴퓨터가 사용하는 마커 ('X' 또는 'O')
 * @param {string} playerMarker - 플레이어가 사용하는 마커 ('X' 또는 'O')
 * @returns {number} 선택된 위치 번호 (1 ~ 9)
 */
function getStrategicPosition(board, computerMarker, playerMarker) {
  // 1단계: 컴퓨터가 이길 수 있는 자리가 있는지 (공격)
  for (let i = 1; i <= 9; i++) {
    if (board[i] === " ") {
      board[i] = computerMarker;
      if (checkWin(board, computerMarker)) {
        board[i] = " "; // 복원
        return i; // 즉시 승리 가능 → 선택
      }
      board[i] = " "; // 복원
    }
  }

  // 2단계: 상대가 이길 수 있는 자리가 있는지 (수비)
  for (let i = 1; i <= 9; i++) {
    if (board[i] === " ") {
      board[i] = playerMarker;
      if (checkWin(board, playerMarker)) {
        board[i] = " "; // 복원
        return i; // 상대의 승리를 막기 위해 선택
      }
      board[i] = " "; // 복원
    }
  }

  // 3단계: 중앙이 비어 있다면 우선 선택
  if (board[5] === " ") {
    return 5;
  }

  // 4단계: 코너가 비어 있다면 순서대로 선택 (1, 3, 7, 9)
  ...
  while (true) {
    ...
    while (gameOn) {
      ...
      // Player 2의 턴 (컴퓨터 또는 AI)
        ...
        let position = getStrategicPosition(theBoard, player2_marker); // 전략형 AI 사용 시
