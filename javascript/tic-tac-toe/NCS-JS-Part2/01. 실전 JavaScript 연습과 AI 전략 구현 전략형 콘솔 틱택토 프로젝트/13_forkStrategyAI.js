...
/**
 * 컴퓨터의 전략적 위치 선택 함수
 * (공격 → 수비 → 포크 → 중앙 → 코너 → 랜덤 순서)
 *
 * - 1순위: 컴퓨터가 이길 수 있는 자리
 * - 2순위: 플레이어가 이길 수 있는 자리 차단
 * - 3순위: 포크(한 수로 2개 이상의 승리 기회를 만드는 자리)
 * - 4순위: 중앙(5번 자리)
 * - 5순위: 코너(1, 3, 7, 9)
 * - 6순위: 랜덤 빈 칸
 *
...
function getStrategicPosition(board, computerMarker, playerMarker) {
  ...
  // 3단계: 포크 전략 - 컴퓨터가 한 번에 두 개 이상의 승리 기회를 만들 수 있는 자리
  for (let i = 1; i <= 9; i++) {
    if (board[i] === " ") {
      board[i] = computerMarker; // ❗ marker → computerMarker 로 수정
      const chances = countWinningChances(board, computerMarker); // 포크 수 계산
      board[i] = " ";
      if (chances >= 2) {
        return i;
      }
