...
/**
 * 컴퓨터의 전략적 위치 선택 함수 (공격 + 수비 + 중앙 우선 전략 포함)
 *
 * - 1순위: 컴퓨터가 이길 수 있는 자리가 있는 경우 → 즉시 선택
 * - 2순위: 플레이어가 다음 수에 이길 수 있는 자리가 있다면 → 차단
 * - 3순위: 중앙(5번 위치)이 비어 있다면 → 중앙 차지
 * - 4순위: 위 조건이 모두 해당되지 않으면 → 랜덤한 빈 칸 선택
 *
 * 이 함수는 간단한 전략 기반 AI로, 공격과 방어를 동시에 수행하고
 * 중앙을 선점하여 승률을 높이는 방식입니다.
 *
 * @param {string[]} board - 현재 보드 배열 (인덱스 1~9 사용)
 * @param {string} computerMarker - 컴퓨터의 마커 ('X' 또는 'O')
 * @param {string} playerMarker - 플레이어의 마커 ('X' 또는 'O')
 * @returns {number} 컴퓨터가 선택한 위치 (1 ~ 9)
 */
function getStrategicPosition(board, computerMarker, playerMarker) {
  // 1단계: 컴퓨터가 이길 수 있는 자리를 먼저 찾기
  for (let i = 1; i <= 9; i++) {
    if (board[i] === " ") {
      board[i] = computerMarker; // 임시 배치

      if (checkWin(board, computerMarker)) {
        board[i] = " "; // 복원
        return i; // 이길 수 있는 자리 → 즉시 선택
      }
  ...
  // 2단계: 플레이어가 이길 수 있는 자리를 찾아 차단
  for (let i = 1; i <= 9; i++) {
    if (board[i] === " ") {
      board[i] = playerMarker; // 플레이어가 둘 경우 시뮬레이션

      if (checkWin(board, playerMarker)) {
        board[i] = " "; // 복원
        return i; // 플레이어가 이길 자리 → 차단
      }
  ...
  // 3단계: 중앙 자리가 비어 있다면 우선 선택 (전략상 유리함)
  if (board[5] === " ") {
    return 5;
  }

  // 4단계: 위 조건이 없으면 랜덤한 빈 칸 선택
  return getRandomEmptyPosition(board);
}
