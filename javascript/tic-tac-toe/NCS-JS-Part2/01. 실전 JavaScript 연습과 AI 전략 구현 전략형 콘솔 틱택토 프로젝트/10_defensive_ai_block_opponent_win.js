...
function getStrategicPosition(board, computerMarker, playerMarker) {
  ...
  for (let i = 1; i <= 9; i++) {
    if (board[i] === " ") {
      board[i] = computerMarker; // 임시로 마커 배치

      if (checkWin(board, computerMarker)) {
        board[i] = " "; // 시뮬레이션 후 복구
        return i; // 승리 가능한 자리 발견 → 즉시 반환
      }
  ...
  // 2단계: 플레이어가 다음 수에 이길 수 있는 자리를 찾아 차단
  for (let i = 1; i <= 9; i++) {
    if (board[i] === " ") {
      board[i] = playerMarker; // 플레이어 마커로 시뮬레이션

      if (checkWin(board, playerMarker)) {
        board[i] = " "; // 복구
        return i; // 플레이어가 이길 수 있는 자리 → 차단
      }
  ...
  // 3단계: 이길 수도 없고, 막을 것도 없다면 랜덤으로 빈 칸 선택
  return getRandomEmptyPosition(board);
}
